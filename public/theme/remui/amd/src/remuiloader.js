...
define(['jquery', 'theme_remui/feedbackcollection', 'core/config'], function ($, feedbackcollection, Config) {
    ...
    const registerCommonEvents = () => {
        ...
        // Handle online users block visibility toggle for edw-icons.
        // The core block toggles fa-eye / fa-eye-slash, but RemUI uses edw-icon-* classes.
        $(document).on('click', '#change-user-visibility', function () {
            var $icon = $(this).find('.icon');

            // Simply swap the edw-icon class that is currently present.
            if ($icon.hasClass('edw-icon-Show')) {
                $icon.removeClass('edw-icon-Show').addClass('edw-icon-Hide');
            } else {
                $icon.removeClass('edw-icon-Hide').addClass('edw-icon-Show');
            }
        });
    };
