...
/**
 * Tracks whether the footer customizer has finished its initial load.
 * Used to avoid overwriting stored default URLs on first render.
 */
let initialized = false;

/**
 * Stored default URLs captured on init so they can be restored after any
 * async post-load overwrites (e.g. file-manager observers).
 */
let initialFooterWidgetLogoDefault = '';
let initialFooterBackgroundImgDefault = '';

function restoreStoredDefaults() {
    if (initialFooterWidgetLogoDefault) {
        $('[name="footerwidgetlogodefault"]').val(initialFooterWidgetLogoDefault);
        let iframeDocument = Utils.getDocument();
        let $logoImg = $(iframeDocument).find("#page-footer #footer-column-1 .footerwidgetlogo");
        if ($logoImg.length > 0) {
            $logoImg.attr("src", initialFooterWidgetLogoDefault).removeClass(SELECTOR.DNONE);
        }
    }

    if (initialFooterBackgroundImgDefault) {
        $('[name="footerbackgroundimgdefault"]').val(initialFooterBackgroundImgDefault);
        let content = `
        #page-footer:not(:has(.section-footer-design-1)) .footer-mainsection-wrapper:before {
            background-image: url('${initialFooterBackgroundImgDefault}') !important;
        }
        `;
...
function footerColors() {
    ...
    let designNumber = parseInt(footerdesign.replace('footerdesign', ''), 10);
    let sectionClass = 'section-footer-design-' + (designNumber + 1);
    ...
    let content = `
            ...
            #page-footer .footer-mainsection-wrapper .${sectionClass} .edw-icon,
            #page-footer .footer-mainsection-wrapper .${sectionClass} i,
            ...
            #page-footer .footer-mainsection-wrapper .${sectionClass} .edw-icon:hover,
            #page-footer .footer-mainsection-wrapper .${sectionClass} i:hover,
...
function applyBackgroundImgURL(defaultImg = true, presetName = false){
    ...
    if(backgroundimg == ''){
        let response = '';
        let storedDefault = $('[name="footerbackgroundimgdefault"]').val();
        if (storedDefault && !initialized) {
            response = storedDefault;
        } else {
            ...
            if(response) {
                if(presetName == "preset-modern") {
                    response = response.replace(/(\.[^.]+)$/, '-modernpreset$1');
                }
                else if(presetName == "preset-classic") {
                    response = response.replace(/(\.[^.]+)$/, '-preset-classic$1');
                }
            }
        }

        // Store computed default URL for PHP to read when no custom background image is uploaded.
        $('[name="footerbackgroundimgdefault"]').val(response);
        ...
        Utils.getFileURL(backgroundimg).done(function(response) {
            if (response == "") {
                ...
                // Apply preset suffix regardless of whether we used storedDefault or computed fresh.
                if(response) {
                    if(presetName == "preset-modern") {
                        response = response.replace(/(\.[^.]+)$/, '-modernpreset$1');
                    }
                ...
                // Custom file uploaded — clear the default store so PHP uses the file URL.
                $('[name="footerbackgroundimgdefault"]').val('');
            }
...
function apply() {
    ...
    // Mark initial load as complete so stored default URLs are no longer preferred.
    let isFirstApply = !initialized;
    initialized = true;

    // Restore any stored defaults that may have been overwritten by async init callbacks.
    // Only run the restoration cascade on the first apply() — later calls (e.g. design switch)
    // intentionally compute new defaults and must not be overwritten.
    if (isFirstApply) {
        restoreStoredDefaults();
        setTimeout(restoreStoredDefaults, 100);
        setTimeout(restoreStoredDefaults, 500);
        setTimeout(restoreStoredDefaults, 1500);
    }
}
...
function updateFooterColumnContent(columnNumber, content) {
    ...
    if (columnNumber === '1' && !initialized) {
        useFooterWidgetLogo(); // If it's column 1 and still during init
    }
}
 ...
 */
function init() {
    // Capture stored defaults before any observers can overwrite them.
    initialFooterWidgetLogoDefault = $('[name="footerwidgetlogodefault"]').val();
    initialFooterBackgroundImgDefault = $('[name="footerbackgroundimgdefault"]').val();
    ...
    Utils.fileObserver(
        ...
        function() {
            let newVal = window.backgroundimgitemid;
            if ($(SELECTOR.BACKGROUNDIMG).val() !== newVal) {
                $(SELECTOR.BACKGROUNDIMG).val(newVal);
    ...
    Utils.fileObserver(
        ...
        function() {
            let newVal = window.footerwidgetlogoitemid;
            if ($(SELECTOR.FOOTERWIDGETLOGO).val() !== newVal) {
                $(SELECTOR.FOOTERWIDGETLOGO).val(newVal);
                useFooterWidgetLogo();
            }
        }
    );
...
function useFooterWidgetLogo(defaultImg = true, applyDarkLogo = false) {
    let iframeDocument = Utils.getDocument();
    let show = $(SELECTOR.SHOWFOOTERWIDGETLOGO).is(":checked");
    ...
    let response = '';
    if(itemid == ''){
        let storedDefault = $('[name="footerwidgetlogodefault"]').val();
        if (storedDefault && !initialized && defaultImg !== false) {
            response = storedDefault;
        } else {
            if(getActiveFooterDesign() === 'footerdesign1') {
            ...
            if (applyDarkLogo && response) {
                response = response.replace(/(\.[^.]+)$/, '-modernpreset$1');
            }
        }

        // Store computed default URL for PHP to read when no custom logo is uploaded.
        $('[name="footerwidgetlogodefault"]').val(response);
        ...
        Utils.getFileURL(itemid).done(function(response) {
            if (response == "") {
                ...
                // Custom file uploaded — PHP uses the file URL first, so keep the stored
                // default intact so it survives if the file is later deleted.
            }
