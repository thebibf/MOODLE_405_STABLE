...
function triggerResize() {
    try {
        ...
        // Sanitize more dropdowns before resize: every child must have a
        // .dropdown-item descendant so core/moremenu's moveOutOfMoreDropdown
        // (which assumes the class is present) does not crash on null.
        doc.querySelectorAll('[data-region="moredropdown"]').forEach(dropdown => {
            Array.from(dropdown.children).forEach(item => {
                if (item.nodeType !== 1 || item.dataset.region === 'morebutton') {
                    return;
                }
                if (!item.querySelector('.dropdown-item')) {
        ...
        // Prevent moremenu crash from propagating to caller.
    }
}
