 ...
 * radio element.
 */
class radio_modified extends base {
    /**
     * Process form save
     *
    ...
    public function get_help($withdefault = true) {
        ...
        if (
            ...
            $help .= '<strong>' . get_string('default', 'moodle') . ': ' . $value['label'] . '</strong><br>';
        }
    ...
    private function prepare_radio_options($options, $default) {
        ...
        foreach ($options as $key => $value) {
            ...
            // Only set selected if default is not false and matches the option name.
            if ($default !== false && $value['name'] == $default) {
    ...
    public function output() {
        ...
        $groupname = isset($options['groupname']) ? $options['groupname'] : $this->name;
        return $OUTPUT->render_from_template($this->component . '/customizer/elements/radio_modified', [
            'name' => $this->name,
            'label' => $label,
            'groupname' => $groupname,
            'help' => $this->get_help(),
