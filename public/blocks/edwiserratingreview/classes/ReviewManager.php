...
class ReviewManager {
     ...
     */
    public function get_reviews_by_filter($start, $length, $order = 'desc', $mindate = '', $maxdate = '', $loadpending = '', $sortcolumn = 'date') {
        ...
        if ($sortcolumn === 'coursename') {
            $sql = "SELECT r.* FROM {$table} r LEFT JOIN {$coursetable} c ON r.for_id = c.id {$whereclause} {$approval} {$datesqlfilter} ORDER BY c.fullname {$order}";
        } else {
    ...
    public function get_approval_compatible_data($reviews) {
        ...
        foreach ($reviews as $key => $review) {
            ...
            $temp["reviewfor"] = "<div class='ellip ellip-5 sorting_1'>" . format_text($this->get_coursename($review->for_id), FORMAT_HTML) . "</div>";
            $temp["name"] = $user->firstname . " " . $user->lastname;
            $temp["email"] = $user->email;
            $temp["review"] = "<div class='ellip ellip-5'>" . (($review->review == "") ? "-" : format_text($review->review, FORMAT_HTML) ) . "<div>";
    ...
    public function get_short_design_enrolmentpage($forid) {
        ...
        $html .= '<div class="d-flex align-items-center justify-content-left rating-short-design" style="color:orange;">';
        $html .= "<div class='d-flex align-items-center'>";
        $html .= "<span class='avgrating h-semibold-6 d-flex'>$data->averagerating</span>" . $data->avergeratingstar . "</div>";
        $html .= "<span class='rnr-link h-semibold-6 d-flex p-pl-0d5'>({$data->totalcount})</span></div>";
