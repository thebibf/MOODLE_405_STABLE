...
// Free components whitelist - components from files loaded in editor.php (lines 50-54)
// All components from: components-html.js, components-elements.js, section.js, components-bootstrap5.js, components-bootstrap4.js
const FREE_COMPONENTS = [
	// From components-bootstrap4.js
	"html/container", "html/gridrow", "html/button", "html/buttongroup", "html/buttontoolbar",
	"html/heading", "html/image", "html/jumbotron", "html/alert", "html/card", "html/listgroup",
	"html/hr", "html/taglabel", "html/badge", "html/progress", "html/navbar", "html/breadcrumbs",
	"html/pagination", "html/form", "html/textinput", "html/textareainput", "html/selectinput",
	"html/fileinput", "html/checkbox", "html/radiobutton", "html/table", "html/paragraph",
	"html/link", "html/video",
	// From components-html.js
	"html/blockquote", "html/list", "html/preformatted", "html/audio", "html/iframe",
	// From components-elements.js
	"elements/font-icon", "elements/carousel", "elements/gallery", "elements/slider",
	"elements/tabs", "elements/accordion", "elements/flip-box", "elements/counter",
	"elements/svg-icon", "elements/figure", "elements/social-icons", "elements/code",
	"elements/image-compare",
	// From components-bootstrap5.js
	"html/btn", "html/btn-link",
	// From section.js
	"elements/section", "elements/header", "elements/footer",
	// From sections-bootstrap4.js
	"bootstrap4/about-team", "bootstrap4/portfolio-one-column", "bootstrap4/portfolio-two-column",
	"bootstrap4/portfolio-three-column", "bootstrap4/portfolio-four-column",
    // From components-edwiser.js
	"html/courses", "html/categories"
];
...
Vvveb.WysiwygEditor = {
	...
	init: function(doc) {
		...
		// Handle dropdown toggle for Boost theme compatibility and dropdown item clicks
		// This works for both Boost and RemUI themes
		const dropdownMenuButton = document.getElementById("dropdownMenuButton");
		const justifyBtn = document.getElementById("justify-btn");

		if (dropdownMenuButton && justifyBtn) {
			// Handle dropdown toggle button click (Boost theme compatibility)
			dropdownMenuButton.addEventListener("click", function (e) {
				const dropdown = this.closest('.dropdown');
				const menu = justifyBtn;
				const isOpen = dropdown.classList.contains('show');

				// Manually toggle dropdown for Boost theme compatibility
				// This ensures it works even if Bootstrap doesn't handle it
				if (isOpen) {
				...
				// Prevent default to avoid double-toggling with Bootstrap
				e.preventDefault();
				e.stopPropagation();
			});
			...
			justifyBtn.addEventListener("click", function (e) {
				...
				if (dropdownItem && dropdownItem.dataset.value) {
					//let command = "justify" + dropdownItem.dataset.value;
					//doc.execCommand(command,false,"#");

					self.editorSetStyle(false, {"text-align" : dropdownItem.dataset.value});

					// Close dropdown after selection
					const dropdown = dropdownMenuButton.closest('.dropdown');
					dropdown.classList.remove('show');
					justifyBtn.classList.remove('show');
			...
			// Close dropdown when clicking outside (Boost theme compatibility)
			// Scoped to only affect this specific dropdown to avoid conflicts with other dropdowns
			document.addEventListener("click", function (e) {
				// Only close if dropdown is currently open
				const dropdown = dropdownMenuButton.closest('.dropdown');
				if (dropdown && dropdown.classList.contains('show')) {
					// Check if click is outside this specific dropdown
					if (!e.target.closest('#wysiwyg-editor .dropdown') &&
						!e.target.closest('#dropdownMenuButton') &&
						e.target !== dropdownMenuButton) {
						dropdown.classList.remove('show');
						justifyBtn.classList.remove('show');
						dropdownMenuButton.setAttribute('aria-expanded', 'false');
					}
	...
	loadControlGroups : function() {
		...
		componentsList.forEach(function (list, i) {
			...
			for (i in components) {
				...
				if (component) {
					// Check if component is in free list, if not and license not active, add lock icon
					let lockIcon = '';
					let proclass = "";
					let title = "";
					if (typeof window._v9 !== 'undefined' && !window._v9 && FREE_COMPONENTS.indexOf(componentType) === -1) {
						lockIcon = '<div class="epb-block-lock-wrapper"><i class="fa fa-lock font-size-14 epb-pro-locked-primary-color"></i></div>';
						componentType = '';
						proclass = "epb-pro-locked";
						title = "Available in Pro";
					}

					item = generateElements(`<li data-section="${group}" class="${proclass}" title="${title}" data-drag-type="component" data-type="${componentType}" data-search="${component.name.toLowerCase()}">
					...
					if (component.image) {
						...
						if (img){
							img.setAttribute("src", Vvveb.imgBaseUrl + component.image);
						}
					}
	...
	loadBlockGroups : function() {
		...
		blocksList.forEach(function (list, i) {
			...
			for (i in blocks) {
				...
				if (block) {
					// Check if license is active, if not add lock icon
					let lockIcon = '';
					let proclass = "";
					let title = "";
;					if (typeof window._v9 !== 'undefined' && !window._v9) {
						lockIcon = '<div class="epb-block-lock-wrapper"><i class="fa fa-lock  epb-pro-locked-primary-color"></i></div>';
						blockType = '';
						proclass = "epb-pro-locked";
						title = "Available in Pro";
					}

					item = generateElements(`<li data-section="${group}" class="${proclass}" title="${title}" data-drag-type="block" data-type="${blockType}" data-search="${block.name.toLowerCase()}">
...
/* iframe */
    ...
    _frameLoaded : function() {
		...
		self.frameHead.append(generateElements('<link data-vvveb-helpers href="' + Vvveb.baseUrl + '../../styles/vvvebjs-editor-helpers.css" rel="stylesheet">')[0]);
...
Vvveb.Gui = {
	...
	preview : function () {
		...
		// Sync active state with button (use setTimeout to ensure it runs after Bootstrap handlers)
		const previewBtn = document.getElementById("preview-btn");
		if (previewBtn) {
			setTimeout(function() {
				// Toggle based on actual preview state, not current class
	...
	fullscreen : function () {
		...
		// Note: Active state will be synced via fullscreenchange event listener
	},
	...
	toggleLeftColumn: function () {
		const visible = Vvveb.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
		// Sync active state with button (use setTimeout to ensure it runs after Bootstrap/jQuery handlers)
		const leftColumnBtn = document.getElementById("toggle-left-column-btn");
		if (leftColumnBtn) {
			setTimeout(function() {
				if (visible) {
					leftColumnBtn.classList.add("active");
					leftColumnBtn.setAttribute("aria-pressed", "true");
				} else {
					leftColumnBtn.classList.remove("active");
					leftColumnBtn.setAttribute("aria-pressed", "false");
				}
			}, 0);
		}
	},
	...
	toggleRightColumn: function (rightColumnEnabled = null) {
		...
		// Sync active state with button (use setTimeout to ensure it runs after Bootstrap/jQuery handlers)
		const rightColumnBtn = document.getElementById("toggle-right-column-btn");
		if (rightColumnBtn) {
			setTimeout(function() {
				if (rightColumnEnabled) {
					rightColumnBtn.classList.add("active");
					rightColumnBtn.setAttribute("aria-pressed", "true");
				} else {
					rightColumnBtn.classList.remove("active");
					rightColumnBtn.setAttribute("aria-pressed", "false");
				}
			}, 0);
		}
...
Vvveb.StyleManager = {
	...
	setStyle: function(element, styleProp, value) {
		...
		// Determine media type based on viewport
		let media = document.getElementById("canvas").classList.contains("tablet") ? "tablet" : document.getElementById("canvas").classList.contains("mobile") ? "mobile" : "desktop";

		// For non-PRO users, always save to desktop regardless of current viewport
		if (typeof window._v9 !== 'undefined' && !window._v9) {
			media = "desktop";
		}
	...
	generateCss: function (media) {
		...
		// Get existing CSS before generating new one (to preserve keyframes and custom media queries)
		let existingCss = this.cssContainer.innerHTML || this.cssContainer.textContent || '';

		// Check if PRO license is active
		const isProActive = typeof window._v9 !== 'undefined' && window._v9;

		// Generate new CSS from library-managed styles
		let css = "";
		for (media in this.styles) {
			// For non-PRO users, only generate desktop CSS (skip mobile/tablet)
			if (!isProActive && (media === "tablet" || media === "mobile")) {
				continue;
			}
			...
			for (selector in this.styles[media]) {
				...
				for (property in this.styles[media][selector]) {
					...
					// Handle empty values: skip them or use default value
					if (value === '' || value === null || value === undefined || value === 'undefined') {
						// For border properties, use 'initial' as default to maintain structure
						// For other properties, skip empty values entirely
						if (property.startsWith('border-')) {
							css += `\t${property}: initial;\n`;
						}
						// Skip other empty properties to avoid invalid CSS
					} else {
						css += `\t${property}: ${value};\n`;
					}
				}
		...
		// Extract keyframes from existing CSS (standalone, not inside media queries)
		// This regex matches @keyframes blocks that are not inside @media queries
		const keyframeRegex = /@keyframes\s+[^{]+\{(?:(?!(?:@media\s+|@keyframes\s+))[^{}]*\{[^{}]*\})*[^{}]*\}/g;
		let keyFrameBlock = existingCss.match(keyframeRegex);

		// Extract custom media queries (those NOT managed by the library)
		// Exclude media queries for tablet/mobile breakpoints that the library manages
		// Extract numeric values from mobileWidth and tabletWidth for dynamic detection
		// Also support old breakpoint values (599px/770px) for backward compatibility
		let mobileWidthNum = this.mobileWidth.replace('px', '');
		let tabletWidthNum = this.tabletWidth.replace('px', '');
		const mediaQueryRegex = new RegExp(`@media\\s+(?!screen\\s+and\\s+\\(max-width:\\s*(?:${mobileWidthNum}px|${tabletWidthNum}px|599px|770px|320px|768px)\\))[^{]*\\{(?:[^{}]*\\{(?:[^{}]*\\{[^{}]*\\})*[^{}]*\\})*[^{}]*\\}`, 'g');
		let mediaQueryBlocks = existingCss.match(mediaQueryRegex);

		// Remove keyframes that are already inside custom media queries (to avoid duplicates)
		...
		if (mediaQueryBlocks && Array.isArray(mediaQueryBlocks)) {
			...
			if (keyFramesInsideMediaQuery && keyFrameBlock) {
		...
		// Ensure arrays exist (handle null/undefined cases)
		if (!mediaQueryBlocks) {
			mediaQueryBlocks = [];
		}
		if (!keyFrameBlock) {
		...
		// Join custom media query blocks
		let mediaQueryBlocksJoined = '';
		if (mediaQueryBlocks && Array.isArray(mediaQueryBlocks) && mediaQueryBlocks.length > 0) {
			mediaQueryBlocksJoined = mediaQueryBlocks.join('\n\n');
		}

		// Combine: keyframes + new library CSS + custom media queries
		// This preserves existing CSS while allowing the library to manage its own styles
		const finalCss = (keyFrameBlock.length > 0 ? keyFrameBlock.join('\n\n') + '\n\n' : '') +
		                 css +
		                 (mediaQueryBlocksJoined ? '\n\n' + mediaQueryBlocksJoined : '');

		this.cssContainer.innerHTML = finalCss;
		return this.cssContainer;
	},
