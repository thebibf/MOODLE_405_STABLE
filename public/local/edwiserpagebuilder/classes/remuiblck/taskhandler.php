...
/**
 * Task handler class for remuiblck.
 *
...
/**
 * This class will handle every operations related to tasks.
 */
class taskhandler {
    /** @var int Task ID. */
    private $taskid;

    /** @var stdClass|null Task object. */
    private $task = null;
    ...
    public function __construct($taskid = -1, $task = null) {
        ...
        if ($taskid != -1 && $task == null) {
            $this->task = $DB->get_record('local_edwiserpagebuilder_taskslist', ['id' => $taskid]);
        } else if ($task != null) {
            $this->task = $task;
        }
    }
    ...
    private function is_mtn_in_any_course($userid = 0) {
        ...
        [$insql, $inparams] = $DB->get_in_or_equal(["manager", "editingteacher", "teacher"], SQL_PARAMS_NAMED);
        ...
        return $DB->get_records_sql($sql, array_merge(['contextlevel' => CONTEXT_COURSE, 'userid' => $userid], $inparams));
    }
    ...
    private function get_users_from_courses($courses) {
        global $DB;
        $namefields = get_all_users_name_fields_rmblck(true, 'u');
        ...
        $sql = "SELECT DISTINCT(u.id), $namefields, u.email
                 ...
                 WHERE c.id $insql
                   ...
                   AND u.deleted = 0
                   AND u.suspended = 0";
        return $DB->get_records_sql($sql, array_merge(['contextlevel' => CONTEXT_COURSE], $inparams));
    }
    ...
    public function get_users() {
        ...
        if (is_siteadmin($USER->id) || has_capability('moodle/site:configview', context_system::instance(), $USER->id)) {
            $namefields = get_all_users_name_fields_rmblck(true);
    ...
    public function get_task_users_details() {
        ...
        if (count($userids) > 4) {
            [$userids, $more] = $this->get_sliced_users($userids);
        }
        $users = $DB->get_records_list('user', 'id', $userids);
        foreach ($userids as $key => $id) {
            $user = new stdClass();
            $user->name = fullname($users[$id]);
            $user->profile = $this->get_user_picture($users[$id], 40)->__toString();
            $userids[$key] = $user;
        }
        if (isset($more)) {
            $user = new stdClass();
    ...
    private function get_message($type) {
        ...
        if (in_array($type, ['create', 'incomplete', 'edit', 'added', 'removed'])) {
            $userfullnames           = $this->get_users_name_exploded_list();
            $bodyparam['assignedto'] = implode(', ', $userfullnames);
        }

        $subject = get_string($type . 'subject', 'local_edwiserpagebuilder', $subjectparam);
        $body    = get_string($type . 'message', 'local_edwiserpagebuilder', $bodyparam);
     ...
     */
    public function notify_users($type, $users = []) {
        global $USER, $PAGE;
    ...
    public function complete($status) {
        ...
        $result = $DB->update_record('local_edwiserpagebuilder_taskslist', $task);
        return $result;
    }
    ...
    public function delete() {
        ...
        $task->deleted = true;
        $task->timemodified = time();
        return $DB->update_record('local_edwiserpagebuilder_taskslist', $task);
    }
    ...
    public function update($task) {
        global $DB;
        $status = $DB->update_record('local_edwiserpagebuilder_taskslist', $task);
        if ($task->notify) {
            [$added, $removed, $nochange] = $this->assignee_changed($task);
    ...
    public static function get_task_sql() {
        ...
        $sql = 'SELECT id, subject, summary, createdby, completed, visible, timedue, assignedto
                  FROM {local_edwiserpagebuilder_taskslist}
                 WHERE deleted <> 1
