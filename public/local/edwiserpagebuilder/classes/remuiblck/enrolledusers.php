...
/**
 * This class will handle every operations related to enrolled users.
 */
class enrolledusers {
    /**
    ...
    /**
     * Check if user can view enrolled users.
     *
    ...
    /**
     * Get enrolled users by category.
     *
    ...
    public static function get_enrolled_users_by_category($categoryid) {
        ...
        if ($courselist) {
            foreach ($courselist as $course) {
                ...
                $count = $DB->get_records_sql($query, [$context->id, '%student%']);
                $count = key($count);
            ...
            $colors = [
                '#264485', '#0051F9', '#37BE71', '#4caf50', '#8bc34a',
                '#ffeb3b', '#ff9800', '#f44336', '#9c27b0', '#673ab7', '#3f51b5',
            ];
