...
/**
 * User handler class for remuiblck.
 *
 ...
 */
class userhandler {
    /** @var userhandler|null Singleton instance. */
    ...
    /**
     * Get latest member data for template.
     *
     * @return array
     */
    ...
    public function get_recent_user() {
        ...
        $users = $DB->get_records_sql(
            'SELECT u.* FROM {user} u  WHERE u.deleted = 0 AND id != 1 ORDER BY timecreated desc',
            [1],
            $limitfrom,
            $limitto
        );
        ...
        foreach ($users as $value) {
            ...
            $userdata[$count]['img'] = $OUTPUT->user_picture($value, ['size' => 100]);
    ...
    public static function get_user($userorid = false) {
        ...
        if (is_object($userorid)) {
            ...
            throw new coding_exception(get_string('parametermustbeobjectorintegerorstring', 'local_edwiserpagebuilder', $userorid));
        }
    ...
    /**
     * Get enrolled users state and latest members.
     *
     * @return array
     */
    ...
    public function get_quiz_stats() {
        ...
        $templatecontext = [];
        // Quiz stats.
        $sqlq = "SELECT DISTINCT q.course courseid, c.shortname shortname, c.fullname fullname"
        ...
        foreach ($coursesforquiz as $course) {
            ...
            if ($isfirstitem) {
                $sqlq = "SELECT DISTINCT q.id quizid, q.name quizname, q.course courseid"
                    . " FROM {quiz} q WHERE q.course = $course->courseid";
    ...
    public function get_user_roles_system_wide() {
        ...
        $sql = "SELECT DISTINCT(r.archetype)
                 ...
                 WHERE ra.userid = ?
                   AND ra.roleid = r.id";
        $roles = $DB->get_records_sql($sql, [$USER->id]);
