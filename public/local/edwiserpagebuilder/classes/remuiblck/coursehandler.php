...
/**
 * Course handler class for remuiblck.
 *
...
/**
 * This class will handle every operations related to courses.
 */
class coursehandler {
    /**
     * Static property to store singleton object.
     *
    ...
    /**
     * Private constructor of class to prevent direct instance creation.
     */
    private function __construct() {
    }

    /**
     * Get singleton instance of coursehandler class.
     *
            ...
            if (!empty($grades)) {
                ...
                foreach ($grades as $grade) {
                    ...
                    if (!$grade->is_hidden() || $canviewhiddengrade) {
                        ...
                        $data[$count]->courseurl = $courseurl->out();
    ...
    public function all_ungraded($userid) {
        ...
        usort($grading, [$this, 'sort_graded']);
    ...
    /**
     * Returns an array of activities which are gradeable from gradebook, outcomes are ignored.
     *
     * @param array  $courseids   Course ids.
     * @param string $modulename If defined then only that type are returned.
     ...
     *
     * @param bool|object $userorid User object or id.
     * @param int         $limit    Number of posts to return.
     * @param int|null    $since    Timestamp to get posts since.
     * @return array Forum activity data.
     */
    public function recent_forum_activity($userorid = false, $limit = 10, $since = null) {
        ...
        $user = \local_edwiserpagebuilder\remuiblck\userhandler::get_user($userorid);
        if (!$user) {
            return [];
        }
        if ($since === null) {
        ...
        if (file_exists($CFG->dirroot . '/local/edwiserpagebuilder/classes/remuiblck/user_forums.php')) {
            require_once($CFG->dirroot . '/local/edwiserpagebuilder/classes/remuiblck/user_forums.php');
        }
        $userforums = new \local_edwiserpagebuilder\remuiblck\user_forums($user, $limit);
        ...
        if (!empty($forumids)) {
            ...
            [$finsql, $finparams] = $DB->get_in_or_equal($forumids, SQL_PARAMS_NAMED, 'fina');
            $params = $finparams;
            $params = array_merge(
                $params,
                [
            ...
            if (!empty($forumidsallgroups)) {
                ...
                [$fgpsql, $fgpparams] = $DB->get_in_or_equal($forumidsallgroups, SQL_PARAMS_NAMED, 'allgpsa');
        ...
        if (!empty($hsuforumids)) {
            [$afinsql, $afinparams] = $DB->get_in_or_equal($hsuforumids, SQL_PARAMS_NAMED, 'finb');
            ...
            if (!empty($hsuforumidsallgroups)) {
                ...
                // user has the ability to access all groups.
                // This will be used in SQL later on to ensure they can see things in any groups.
                [$afgpsql, $afgpparams] = $DB->get_in_or_equal($hsuforumidsallgroups, SQL_PARAMS_NAMED, 'allgpsb');
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                ...
                $userobj = $DB->get_record("user", ["id" => $post->userid]);

                // Update the user object with user profile photo.
                $postuser->profilepicture = $OUTPUT->user_picture($userobj, ['size' => 30]);
                ...
                if (!in_array($post->cmid, $topics)) {
                    ...
                    $topics[] = $post->cmid;
                    $count++;
                    $discussion = $activities[$count]->content->discussion;
                    $activities[$count]->replies = $this->get_postcount_postusers(
                        $posts,
                        $discussion,
                        "postcount"
                    );
                    $activities[$count]->recentuser = array_values(
                        $this->get_postcount_postusers($posts, $discussion, "userspost")
                    );
                } else {
                    $discussion = $activities[$count]->content->discussion;
                    $activities[$count]->replies = $this->get_postcount_postusers(
    ...
    /**
     * Get post count and post users for a discussion.
     *
     * @param array  $posts  Posts array.
     * @param int    $postid Discussion id.
     * @param string $option Option to return postcount or userspost.
     * @return mixed Post count or users array.
     */
     ...
     * @param bool   $loadprogress Whether to load progress.
     ...
     * @param int     $start        Start index of course.
     * @param int     $length       End index of course.
     * @param array   $order        In which order courses should be arranged.
     * @param bool    $loadprogress Load course progress.
     ...
     *
     * @param int|array $courses Single course or course ids.
     * @param int|null  $userid  User id or null if current user.
     * @return int|array True if user is teacher or array of course ids.
     */
    public function user_is_teacher($courses, $userid = null) {
        ...
        $params = array_merge(
            ...
            $inparams,
            ['userid' => $userid, 'archetype' => '%teacher%'],
        );
     ...
     * @return object Last access time.
     */
    public function get_last_course_access_time($courseid, $studentid) {
        ...
        if (!empty($record)) {
            ...
            $lastaccess->class = ($currtime - $record) > 259200
                ? 'text-danger'
                : (($currtime - $record) > 129600 ? 'text-warning' : 'text-success');
        }
        return $lastaccess;
    }
     ...
     * @param array  $order    Sorting column and direction.
    ...
    public function get_filtered_dropping_user_stats($courseid, $search, $table, $start, $length, $order) {
        global $DB;
        $order = (array)$order;
        $columns = ["Name", "u.email", "ue.timestart", "lsl.timecreated"];
    ...
    public function get_user_image_and_link($user, $courseid) {
        ...
        $imagelink = html_writer::tag(
            ...
            '',
            [
                'src' => $this->get_user_picture($user),
                'alt' => $title,
                'title' => $title,
                'class' => 'userpicture',
                'width' => 35,
     ...
     * @param int     $imgsize    Size of image in pixel.
     * @return string User picture link.
     */
    public function get_user_picture($userobject = null, $imgsize = 100) {
        global $USER, $PAGE;
        if (!$userobject) {
            $userobject = $USER;
        }
