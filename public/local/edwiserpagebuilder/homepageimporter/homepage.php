...
$PAGE->set_url(new moodle_url(
    '/local/edwiserpagebuilder/homepageimporter/homepage.php',
    ['data' => $data]
));
...
if ($data == "migrate") {
    ...
    try {
        ...
        $sections = $DB->get_records('remuihomepage_sections', ['visible' => 1]);
        $blockinstances = [];
        $blockseq = 0;
        ...
        if (!$sectionempty) {
            foreach ($sections as $section) {
                ...
                $sectionconfightml->sectionpropertiesoutput = str_replace(
                    'invisible',
                    '',
                    $sectionconfightml->sectionpropertiesoutput
                );
                if ($section->name == 'html') {
                    ...
                    $blockinstances[] = [
                        ...
                        "blockstyles" => '',
                        "blockhtml" => '<div class = "home-sections overflow-hidden">'
                            . $OUTPUT->render_from_template('local_remuihomepage/html', $sectionconfightml)
                            . '</div>',
                    ];
                } else {
                        ...
                        $blockinstances[] = [
                            ...
                            "blockhtml" => '<div class = "home-sections overflow-hidden">'
                                . $OUTPUT->render_from_template(
                                    'local_remuihomepage/' . $section->name,
                                    $sectionconfightml
                                )
                                . '</div>',
                        ];
            ...
            foreach ($blockinstances as $homeblockinstance) {
                ...
                $blockrecord = $DB->get_record(
                    ...
                    [
                        ...
                        "pagetypepattern" => $pagetypepattern,
                        "defaultregion" => $defaultregion,
                        "defaultweight" => $homeblockinstance["blockseq"],
                    ],
                    '*'
                );
                $homeblockinstance["blockhtml"] = preg_replace(
                    '/\sonload\s*=\s*["\'][^"\']*["\']/',
                    '',
                    $homeblockinstance["blockhtml"]
                );
            ...
            $successimg = $CFG->wwwroot . '/local/edwiserpagebuilder/pix/success_icon.svg';
            $successmsg = get_string('homepagemigrationtitlemsg', 'local_edwiserpagebuilder');
            echo("<div class='text-center'>"
                . "<img src='" . $successimg . "' />"
                . "<h4 style='margin: 16px 0;'>" . $successmsg . "</h4>"
                . "</div>");
            $viewhomepage = get_string('viewhomepage', 'local_edwiserpagebuilder');
        ...
        $failimg = $CFG->wwwroot . '/local/edwiserpagebuilder/pix/failer_icon.svg';
        $failmsg = get_string('homepagemigrationfailtitlemsg', 'local_edwiserpagebuilder');
        echo("<div class='text-center'>"
            . "<img src='" . $failimg . "' />"
            . "<h4 style='margin: 16px 0;'>" . $failmsg . "</h4>"
            . "</div>");
        $retryurl = $CFG->wwwroot
            . "/local/edwiserpagebuilder/homepageimporter/homepage.php?data=migrate";
        $tryagain = get_string('tryagain', 'local_edwiserpagebuilder');
        echo "<div class='mt-1 text-center'>"
            . "<a class='btn btn-primary btn-sm' href='"
            . $retryurl . "'>" . $tryagain . "</a></div>";
    }
