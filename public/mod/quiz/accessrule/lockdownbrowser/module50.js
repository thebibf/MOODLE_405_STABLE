    ...
    init: function(Y, start, preview) {
        ...
        require(['core_user/repository'], function(UserRepository) {
            UserRepository.getUserPreference('quiz_timerhidden')
                .then((response) => {
                    M.quizaccess_lockdownbrowser.timer.setVisibility(response !== '1', false);
                    return;
                })
                ...
                .catch((error) => {
                    M.quizaccess_lockdownbrowser.timer.setVisibility(true, false);
                    Y.log(error, 'error', 'moodle-quizaccess_lockdownbrowser');
                });
        });
    },
...
M.quizaccess_lockdownbrowser.secure_window = {
    ...
    init_close_button: function(Y, url) {
        Y.on('click', function(e) {
            M.quizaccess_lockdownbrowser.secure_window.close(Y, url, 0);
        }, '#secureclosebutton');
    },
