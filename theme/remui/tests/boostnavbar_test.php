...
/**
 * Test the remuinavbar file
 *
 * @package    theme_remui
 * @covers     \theme_remui\remuinavbar
 * @copyright  2021 Peter Dias
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class remuinavbar_test extends \advanced_testcase {
    /**
    ...
    public function test_remove_no_link_items(array $setup, bool $removesectionnodes, array $expected) {
        ...
        $remuinavbar = $this->getMockBuilder(remuinavbar::class)
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();

        $rc = new \ReflectionClass(remuinavbar::class);
        $rcp = $rc->getProperty('items');
        $rcp->setAccessible(true);
        $rcp->setValue($remuinavbar, $page->navbar->get_items());

        // Make the call to the function.
        $rcm = $rc->getMethod('remove_no_link_items');
        $rcm->setAccessible(true);
        $rcm->invoke($remuinavbar, $removesectionnodes);

        // Get the value for the class variable that the function modifies.
        $values = $rcp->getValue($remuinavbar);
        $actual = [];
    ...
    public function test_remove_items_that_exist_in_navigation(string $navmenu, array $navmenunodes, array $navbarnodes,
        ...
        $rcm->setAccessible(true);
        $rcm->invoke($remuinavbar, $navigationmenu);
